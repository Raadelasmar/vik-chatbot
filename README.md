...
4. **Data Ingestion**:
   * Place university PDFs in the `/data` folder.
   *  Run `python ingest_database.py` to build the local vector store.

5. **Launch** (Run in two separate terminal windows):
   *  Run the API: `fastapi dev main.py`
   *  Run the UI: `streamlit run ui.py`
